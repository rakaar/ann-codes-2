function [STIMS,stim_structure]=File_RSS_SSA_Generate(stim_structure,HW)
...
nas=1; %%% HARD CODED %%%%stim_structure.stim_protocol.num_att_steps;
...
set=stim_structure.stim_protocol.choose_set;
stim_dur=stim_structure.stim_protocol.stim_duration;
reps=stim_structure.stim_protocol.total_reps;



% following parameters for all stimuli %%%%%%%%%%%%%%%%%%%%%%%
STIMS.duration=stim_dur;
...
strx1='C:\EXPERIMENTS\STIMULUS\RSS_SSA\RSS_SSA_SET';
strx2=num2str(set);
strx3='\STIMULI';
...
for line_num=1:nstims_set*nas
    ...
    eval(sprintf('[stm]=wavread(''rb%03d'');',stnum))
