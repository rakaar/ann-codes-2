function varargout = PrairiePartnerGUI(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @PrairiePartnerGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @PrairiePartnerGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function PrairiePartnerGUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = PrairiePartnerGUI_OutputFcn(hObject, eventdata, handles) 
...
switch wtd_val
    ...
    case 4
        ...
        if AudStimPARAMS.frame_check==1
            if AudStimPARAMS.MultFrame==0
                [finished_stim]=prairie_aud_struct_frames(AudStimPARAMS,handles);
            elseif AudStimPARAMS.MultFrame==1
                [finished_stim]=prairie_aud_struct_multipleframes(AudStimPARAMS,handles);
            end
            if finished_stim==1
                set(handles.start_button,'BackgroundColor',[0.8314    0.8157    0.7843])
            ...
            clk=clock;dt=date;
            %cd G:\MATLAB701\work\2p\DATA\PP_files
            cd 'C:\Documents and Settings\kanoldlab\My Documents\Data\PP_files'
            eval(sprintf('save PP_file_%s_%i_%i_%i-%i PP_PARAMS',dt,clk(4),clk(5),fix(clk(6)),PP_PARAMS.file_number))
            %cd G:\MATLAB701\work\2p\TEST
            cd 'C:\Documents and Settings\kanoldlab\My Documents\MATLAB\PrairiePartner'
        elseif AudStimPARAMS.linescan_check==1
            [finished_stim]=prairie_aud_struct_lines(AudStimPARAMS,handles);
            if finished_stim==1
                set(handles.start_button,'BackgroundColor',[0.8314    0.8157    0.7843])
                set(handles.disp_status,'String','Done Auditory Stimulation')
                load PP_PARAMS
    ...
    case 12
        ...
        [finished_stim]=prairie_pairing_struct(PairingPARAMS,handles);
        if finished_stim==1
                set(handles.start_button,'BackgroundColor',[0.8314    0.8157    0.7843])
...
switch wtd_val
    case 2
        % pwatkins - added this to prevent continuous puff problem
        %daq=daqhwinfo('nidaq');
...
set(handles.start_button,'BackgroundColor',[0.8314    0.8157    0.7843])
%daqreset % pwatkins - was there a reason for this here?
wtd_val=get(handles.what_to_do,'Value');
switch wtd_val
    case 2
        % pwatkins - added this to prevent continuous puff problem
