function [finished_stim,OptoParams]=prairie_Opto_struct(OptoParams,PP_handles)
...
if stim_structureOpto.check_optoSSA==1
    if stim_structureOpto.check_standard==1
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       %%%%%%%%%%%%%%%modulatory square wave 1khz 82% duty cycle for 598nm(Arch)%%%%%%%%
  if OptoParams.check589nm==1
    modulate_time=0:1/HW.Fso2:(stim_structureOpto.stimA_dur);
modulate_wav=square(2*pi*1000*modulate_time,82);%82% duty cycle 1khz square waveform
    one_blinkO=[modulate_wav  zeros(1,fix(stim_structureOpto.isi_dur*HW.Fso2))];
       ...
       one_blinkO=[ones(1,fix(stim_structureOpto.stimA_dur*HW.Fso2))  zeros(1,fix(stim_structureOpto.isi_dur*HW.Fso2))];
 end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         one_blinkO=[ones(1,fix(stim_structureOpto.stimA_dur*HW.Fso2))  zeros(1,fix(stim_structureOpto.isi_dur*HW.Fso2))];
    elseif stim_structure.check_deviant==1
        %%%%%%%%%%%%%%%modulatory square wave 1khz 82% duty cycle for 598nm(Arch)%%%%%%%%
  if OptoParams.check589nm==1
    modulate_time=0:1/HW.Fso2:(stim_structureOpto.stimB_dur);
modulate_wav=square(2*pi*1000*modulate_time,82);%82% duty cycle 1khz square waveform
    one_blinkO=[modulate_wav  zeros(1,fix(stim_structureOpto.isi_dur*HW.Fso2))];
    else
       one_blinkO=[ones(1,fix(stim_structureOpto.stimB_dur*HW.Fso2))];
 end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         one_blinkO=[ones(1,fix(stim_structureOpto.stimB_dur*HW.Fso2))];
    end
...
if stim_structureOpto.check_optoSSA==1
    devpos=stim_structureOpto.stimB_pos;
    allstimsO(((devpos-1)*length(one_blinkO)+1):devpos*length(one_blinkO))=-allstimsO(((devpos-1)*length(one_blinkO)+1):devpos*length(one_blinkO));
end
...
for iter=1:t_iters
    ...
    if interrupt_opto==0
        ...
        if totsamps<length([zeros(start_samp,1);stimuO'])
            ...
            stim_structureOpto.total_time=fix(totsamps/HW.Fso);
            set(PP_handles.disp_status,'String','Adjusted total time to fit in entire stimulus')
            pause(0.2)
        end
        
        HW.SS.queueOutputData(zeroslen2);
    else
    ...
    set(PP_handles.disp_status,'String',current_string)
    if stim_structureOpto.gapcheck&iter~=t_iters
        eval(sprintf('current_string=''DONE (ITER # %i ...); Waiting for ... %5.2f seconds '';',iter,stim_structureOpto.gap_dur))
        set(PP_handles.disp_status,'String',current_string)
        pause(stim_structureOpto.gap_dur)
    end
end
