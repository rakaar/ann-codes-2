function [finished_stim]=prairie_aud_struct_frames(stim_structure,PP_handles)
...
for iter=1:t_iters
    ...
    iStatus = Select_Signal(CTR_BOARD,param.ND_GPCTR0_OUTPUT, param.ND_GPCTR0_OUTPUT, ...
        param.ND_LOW_TO_HIGH);
    % START, Could not get internal routing of the signal to work:
    %iStatus = Select_Signal(CTR_BOARD,param.ND_GPCTR0_OUTPUT, param.ND_RTSI_0, ...
    %    param.ND_LOW_TO_HIGH);
    %iStatus = Select_Signal(CTR_BOARD,param.ND_GPCTR0_OUTPUT, param.ND_NONE, ...
    %    param.ND_DONT_CARE);
    %iStatus = Select_Signal(CTR_BOARD,param.ND_RTSI_0, param.ND_GPCTR0_OUTPUT, ...
    %    param.ND_DONT_CARE); % pwatkins - internally route counter to bus
    %iStatus = Select_Signal(CTR_BOARD,param.ND_IN_START_TRIGGER, param.ND_RTSI_0, ...
    %    param.ND_LOW_TO_HIGH); % pwatkins - route counter on bus to input trigger (in HWInit also)
    %iStatus = Select_Signal(CTR_BOARD,param.ND_PFI_0, param.ND_IN_START_TRIGGER, ...
    %    param.ND_LOW_TO_HIGH); % pwatkins - externally output counter from bus to pfi0
    % END, Could not get internal routing of the signal to work:
    ...
    iStatus=DIG_Line_Config(1,0,5,1); % pwatkins - fake triggers
    ...
    %    /* Loop iLoopCount times. */
     ...
     for count=1:iLoopCount
        ...
        if pulCount.value==(CTR_RESET)
            ...
            load interrupt_aud
            if interrupt_aud==0
                %done = invoke ( RX6_gPL, 'GetTagVal', 'nowPlaying' )
                RX6_gPL.SetTagVal('Enable',0); RX6_gPL.Halt;
                stop([HW.ai])
                %stop([HW.ai HW.ao])
        ...
        %pause(.00001);
        % pwatkins - fake triggers
        iStatus=DIG_Out_Line(1,0,5,1); pause(1e-6); iStatus=DIG_Out_Line(1,0,5,0); pause(0.1);
     end
    %pause;
end
finished_stim=1;
