function [finished_stim]=prairie_aud_struct_intrinsic(stim_structure,PP_handles)
        ...
        if pulCount.value==CTR_TRIG
            ...
            if interrupt_aud==0
                if presented(iter)==0
                    ...
                    if iter<t_iters
                        if STIMS.hard==1
                            eval(sprintf('stimulus=STIMS.waveform.line%i;',line_num+1)) %%%%%%%% Currently written as same stimulus in both ears (if both)
                            stimulus=[stimulus*HW.Amplitude];
                            if stim_structure.left_check==1
                                %sprintf('left')
                                stimulus=[stimulus zeros(size(stimulus))];
