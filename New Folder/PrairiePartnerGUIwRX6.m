...
interrupt_FB=0; save interrupt_FB interrupt_FB
load PP_PARAMS
...
%         uiresume
    ...
    case 15
        ...
        case 17
        set(handles.disp_protocol,'String','FEMTO Behaviour')
        set(handles.disp_status,'String','Not Started')
        PP_handles=handles;
        save Femto_Behaviour_handle PP_handles
        currr=pwd;
        ...
        Femto_Behaviour_GUI %% GUIname
        load Femto_Behaviour_params
%         %set(handles.disp_protocol,'String',VisLEDStimPARAMS.protocol_str)
        clear Femto_Behaviour_params
        uiresume
end
...
%         end
    ...
    case 15
        ...
        elseif OptoParams.check_none==1
            [finished_stim,OptoParams]=prairie_Opto_struct(OptoParams,handles);
     ...
     case 17
        set(handles.disp_status,'String','Started FEMTO Behaviour')
        
        load Femto_Behaviour_params
        interrupt_FB=0;
        save interrupt_FB interrupt_FB
       
        if Femto_Behaviour_params.auditory_only==1
            [finished_stim,Femto_Behaviour_params]=prairie_FBAud_struct(Femto_Behaviour_params,handles);
        elseif Femto_Behaviour_params.visual_only==1
            [finished_stim,Femto_Behaviour_params]=prairie_FBvisLED_struct(Femto_Behaviour_params,handles);
        elseif Femto_Behaviour_params.multisensory==1
            [finished_stim,Femto_Behaviour_params]=prairie_FBvisLEDAUD_struct_RX6(Femto_Behaviour_params,handles);
        elseif OptoParams.reward_only==1
            [finished_stim,Femto_Behaviour_params]=prairie_Opto_struct(Femto_Behaviour_params,handles);
        
        end
        if finished_stim==1
            set(handles.start_button,'BackgroundColor',[0.8314    0.8157    0.7843])
            set(handles.disp_status,'String','Done Behaviour Session')
            load PP_PARAMS
            PP_PARAMS.type='Femto-Behaviour';
            PP_PARAMS.protocol=Femto_Behaviour_params;
            load trig_inputs
            ephysdata=trig_inputs;
            PP_PARAMS.triggers=[]; %change here if we are
            %acquiring trigger inputs
