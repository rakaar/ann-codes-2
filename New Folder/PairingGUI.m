...
% Last Modified by GUIDE v2.5 28-Sep-2014 12:26:41
...
set(handles.ep16check,'Value',PairingPARAMS.ep16check);

eval(sprintf('set(handles.OnsetE_A,''String'',''%i'');',PairingPARAMS.OnsetE_A))
set(handles.OnsetE_A,'Value',PairingPARAMS.OnsetE_A);
...
% --- Executes on button press in Electrical_Episodic_button.
function Electrical_Episodic_button_Callback(hObject, eventdata, handles)
% hObject    handle to Electrical_Episodic_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

load AudStimPARAMS

switch AudStimPARAMS.type
    case 'ABA'
        ABA_EpisodicEStim_GUI
    case 'SSA'
        SSA_EpisodicEStim_GUI
    otherwise 
        fprintf('\nEpisodic Stim Not setup\n')
end


% --- Executes on button press in ep16check.
function ep16check_Callback(hObject, eventdata, handles)
% hObject    handle to ep16check (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of ep16check

load PairingPARAMS
PairingPARAMS.ep16check=get(hObject,'Value'); %returns contents of num_pt_val as a double
set(handles.ep16check,'Value',PairingPARAMS.ep16check);
...
function OnsetE_A_Callback(hObject, eventdata, handles)
% hObject    handle to OnsetE_A (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of OnsetE_A as text
%        str2double(get(hObject,'String')) returns contents of OnsetE_A as a double

load PairingPARAMS
PairingPARAMS.OnsetE_A=str2double(get(hObject,'String')); %returns contents of num_pt_val as a double
set(handles.OnsetE_A,'Value',PairingPARAMS.OnsetE_A);
curdir=pwd;
cd C:\EXPERIMENTS\CODES\PARAMS
save PairingPARAMS PairingPARAMS
...
% --- Executes during object creation, after setting all properties.
function OnsetE_A_CreateFcn(hObject, eventdata, handles)
% hObject    handle to OnsetE_A (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
