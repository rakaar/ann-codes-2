function [finished_stim]=prairie_aud_struct_multipleframes(stim_structure,PP_handles)
...
if stim_structure.max_speed_check==0
    CTR_BOARD=1;CTR_TRIGx=TC-trig_frame;CTR_RESETx=tfcount-trig_frame;pause_del=0.05;
elseif stim_structure.max_speed_check==1
    CTR_BOARD=2;CTR_TRIGx=TC-trig_frame+1;CTR_RESETx=tfcount-trig_frame+1;pause_del=0;
end
...
[CTR_RESETALL,CTR_TRIGALL,STIMFRAMES]=create_ctr_trigreset_set(stim_structure,STIMS.duration,STIMS.lines_per_set,TC);
CTR_TRIGALL(1)=CTR_TRIGx;STIMFRAMES(1)=trig_frame;STIMS.STIMFRAMES=STIMFRAMES;
for rep=1:nreps
    for stimnum=1:STIMS.lines_per_set
        line_num=stimnum;iter=(rep-1)*STIMS.lines_per_set+stimnum;
        CTR_RESET=CTR_RESETALL(stimnum);
        CTR_TRIG=CTR_TRIGALL(stimnum);
        %STIMS.START_FRAMES(stimnum)=
        %%% Display
        eval(sprintf('current_string=''Now at Rep %i of %i, Stim Number %i of %i!'';',rep,nreps,stimnum,STIMS.lines_per_set))
        set(PP_handles.disp_status,'String',current_string)
    ...
    %    /* Loop iLoopCount times. */
        ...
        for count=1:iLoopCount
            ...
            iStatus = GPCTR_Watch(CTR_BOARD, counterNum, param.ND_COUNT, ...
                pulCount);
            if pulCount.value==1
                eval(sprintf('current_string=''Started Stimulus (number # %i ...) at frame # %i'';',stimnum,STIMFRAMES(stimnum)))
                set(PP_handles.disp_status,'String',current_string)
                pause(0.002)
            end
