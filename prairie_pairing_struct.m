...
% CHANGING THIS FOR PAIRING WITH LONG STIMULI
% CONNECTIONS NEED TO BE CHANGED
% DAC1OUT ON BOARD 1 OUTPUTS AUDITORY STIMULUS TO RIGHT CHANNEL
% DAC0OUT ON BOARD 1 OUTPUTS ELECTRICAL STIMULUS
% THIS NEEDED TO BE DONE
% DISCONNECT DAC1OUT OF BOARD 2 FROM THE CONNECTIONS ON/HANGING ABOVE DAC0OUT ON BOARD 1
% CONNECT THE REST TO DAC0OUT ON BOARD 1
% CONNECT USER2 LINE TO MASTER TRIGGER CONNECTIONS
% TURN OFF RX6
% DISCONNECT CONNECTION ON RX6 OUTPUT THE OTHER 3 SHOULD REMAIN CONNECTED
% CHANGES IN CODE MARKED WITH ***
...
%***
pulsewv=resample(pulsewv,HW.Fso1,HW.Fso2);
%***
...
for iter=1:t_iters
    ...
    if interrupt_aud==0
        ...
        stimulus(1:length(pulsewv),1)=pulsewv';
        putdata(HW.ao1,stimulus)
        %         figure(2)
        %         plot(stimulus)
        %         drawnow
    ...
    for count=1:iLoopCount
        ...
        if is_cycle_time_ellapsed % PVW000001
            ...
            if interrupt_aud==0
                ...
                if pulCount.value==(CTR_RESET)
                    %current_string = sprintf('DONE (ITER # %i ...) ',iter);
