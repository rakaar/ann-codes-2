...
% Last Modified by GUIDE v2.5 25-Feb-2015 10:35:51
...
if SSAPARAMS.randB_check==1&SSAPARAMS.rand_nstim_check==0
    SSAPARAMS.rand_stimB_loc=fix(total_stims*rand(1,treps))+1;%%% SB there was error till 13/1/2015 - corrected 14/1/2015 SB
end
if SSAPARAMS.rand_nstim_check==1
    changemax=fix(1/3*total_stims);
    nstimvar=fix((2*changemax+1)*rand(1,treps)+1)-changemax-1;
    SSAPARAMS.rand_nstim_num=repmat([total_stims],1,treps)+nstimvar;%%% 25% variation
    if SSAPARAMS.randB_check==1
        for lll=1:treps
            SSAPARAMS.stimB_loc(lll)=fix(SSAPARAMS.rand_nstim_num(lll)*rand(1,1))+1;
        end
    end
